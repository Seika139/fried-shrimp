...
### 1. ログインエラー（token_mismatch）が発生した場合
`device identity required` や `token_mismatch` が表示されても心配いりません。解決策はシンプルです：
1. ブラウザ上の **Control UI Settings** (設定アイコン) を開く。
2. 以下のトークンをコピーして、トークン欄に貼り付け（保存）してください：
   - **正しいトークン**: `1cdb6211e965ea8c66f5d13896e665288c4332b497857e68`

> [!NOTE]
> 現在、`dangerouslyDisableDeviceAuth: true` を設定しています。これにより、トークンさえ合致すればデバイス認証なしでログインできるようになっています。

### 2. Tailscale の再接続
コンテナ再構築後に認証が切れた場合は、ターミナルで `tailscale up` を実行してログインし直してください。設定が完了したら、以下のURLでダッシュボードにアクセスできます：
